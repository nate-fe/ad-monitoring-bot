...
(function () {  
  ...
  if( self === top ) {
    ...
    try {
        ...
        if (ifm_wrap.length === 0) {
            ifm_wrap = parent.document.querySelectorAll('#top_main_banner');
        }

        ifm_wrap.forEach(function(v) {
            var ifm = v.querySelector('iframe');
            if (ifm) ifm.height = ad_height;
            v.style.height = ad_height + 'px';
            v.style.backgroundColor = bgcolor;
        });
    } catch (e) {
        // 도메인이 달라 부모 문서에 접근 못하는 경우 에러 없이 종료
        console.warn("Parent access blocked (Cross-Origin)");
    }
  }
})();