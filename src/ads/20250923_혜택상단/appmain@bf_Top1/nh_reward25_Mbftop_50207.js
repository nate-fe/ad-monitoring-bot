(function () {
    try { if (!CyadLib) var CyadLib = {}; CyadLib.hasOwnProperty("prefixUrl") || (CyadLib.prefixUrl = function (t) { var r = location.protocol; return /^http[s]?:/.test(r) && (t = t.replace(/http:/g, r).replace(/https:/g, r)), /^\/\/\w+?/.test(t) && (t = r + t), /^http[s]?\/\//.test(t) && (t = t.replace(/http\/\//g, r + "//").replace(/https\/\//g, r + "//")), t }) } catch (t) { }

    var ad_height = '84'; //광고 높이 값
    var bgcolor = '#f5f7fb';
    var img_path = CyadLib.prefixUrl('https://adimg.nate.com/img/2025/04/reward/reward_0407_690x168.png');	//이미지 소재 경로
    var click_path = CyadLib.prefixUrl('http://cyad1.nate.com/click.kti/mnate/main@bf_Top1?ads_no=233982&cmp_no=30687&img_no=414138'); //랜딩
    var alt_text = '광고';
    var ad_id = '#ad_big'; //상단

    var adsEl = '<a href="' + click_path + '" style="display:block;height:' + ad_height + 'px;text-align:center;background:' + bgcolor + '" target="_top" ><img alt="' + alt_text + '" src="' + img_path + '" width="320" height="' + ad_height + '" border="0"></a>';

    if (self === top) {
        var _currentScript = document.currentScript;
        var _adArea = _currentScript.closest('.adloader');
        _adArea.style.backgroundColor = bgcolor;
        _adArea.innerHTML = adsEl;
        _adArea.style.height = ad_height + 'px';
    } else {
        document.body.style.background = bgcolor;
        document.write(adsEl);

        var ifm_wrap = parent.document.querySelectorAll(ad_id);
        ifm_wrap.forEach(v => {
            var ifm = v.querySelector('iframe');
            if (ifm) {
                ifm.height = ad_height;
            }
        });
    }
})()