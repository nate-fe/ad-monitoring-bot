...
var cur_div = _scriptEl.closest('.ad_box_text');
...
var _pubId = isAOS() ? "812394559" : "530066011";
var _plcmtId = isAOS() ? "Ow9lORKN0kxB1hb" : "iydZRtJgZDhEVRB";
var _adUnitId = isAOS() ? "Nate_Ranking_WebFeed_Android-N256497692" : "Nate_Ranking_WebFeed_iOS-N256497692";
...
if (getAgentValue("ref", ua) == app_name) {
    ...
    apsspads.cmd.push(function () {
        if (isAOS()) {
            ...
            console.log("iOS");
            apsspads.setConfig({ pubId: _pubId, nam: true, idfv: _adid }); // 애드팝콘 앱키
            apsspads.defineAdUnits({
                code: _ad_div_id,  // 광고 노출 영역 (HTML Element)의 Id (광고 게재 위치 지정 참고)
                plcmtId: _plcmtId,  // 애드팝콘 지면 Id
                fallback: {
